...
class Posts extends Component {
	...
	render() {
		...
		return (
			<div className="container-fluid postsmainBody">
				<div className="row">
				...
				{this.renderPosts(posts)}
					<div className="paginationCenter">
						{page > 1 ? (
		                    <button
		                        className="btn btn-raised btn-warning mr-3 mb-5 paginationDIV"
		                        onClick={() => this.loadLess(1)}
		                    >
		                     <i class="fa fa-arrow-left"></i> {""} ({this.state.page - 1}) Previous
		                    </button>
		                ) : (
		                    ""
		                )}
		                ...
		                {posts.length ? (
		                    <button
		                        className="btn btn-raised btn-success mb-5 paginationDIV"
		                        onClick={() => this.loadMore(1)}
		                    >
		                        Next ({page + 1}) {""} <i class="fa fa-arrow-right"></i>
		                    </button>
		                ) : (
		                    ""
		                )}
